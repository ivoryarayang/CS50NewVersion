...
//
//void push_th(float val, float* vertex_min_edge, int th_test) {
//    int ind = th_test - 1;
//    while (val < vertex_min_edge[ind]) {
//        ind--;
//    }
//    ind++;
//    if (ind < th_test) {
//        int i;
//        for (i = th_test - 2; i >=ind; i--) {
//            vertex_min_edge[i + 1] = vertex_min_edge[i];
//        }
//        vertex_min_edge[ind] = val;
//    }
//}
...
int generate_graph0d(int n, edge** graph, int sample_size) {
   ...
   /* float** edges;
    ...
    //for (i = 0; i < n; i++) {
    //    for(j=0;j<th_test;j++)
    //        vertex_min_edge[i][j] = 1.0;
    //}

    //float th = 0.0;
    //for (i = 0; i < n; i++) {
    //    th = vmin_edges[i][th_test - 1] > th ? vmin_edges[i][th_test-1]+error_margin:th;
    //}
...
int generate_graph234d(int n,int d, edge** graph, int sample_size) {
    ...
    delete ver_dim;
    return numEdges;
    //const float error_margin = 1e-10;
    //int numEdges = 0;
    //float** edges;
    //edges = (float**)malloc(sizeof(float*) * n);
    //for (i = 0; i < n; i++)
    //    edges[i] = (float*)malloc(sizeof(float) * n);
    //float* vertex_min_edge = (float*)malloc(sizeof(float) * n);
    //for (i = 0; i < n; i++)
    //    vertex_min_edge[i] = 1.0;

    //for (i = 1; i < n; i++) {
    //    for (j = 0; j < i; j++) {
    //        float val = distance(d, ver_dim[i], ver_dim[j]);
    //        edges[i][j] = val;
    //        if (val < vertex_min_edge[i])
    //            vertex_min_edge[i] = val;
    //        if (val < vertex_min_edge[j])
    //            vertex_min_edge[j] = val;
    //    }
    //}
    //float th = 0.0;
    //for (i = 0; i < n; i++) {
    //    th = vertex_min_edge[i] > th ?  vertex_min_edge[i] + error_margin:th;
    //}
    //// Find all edges
    //for (int i = 1; i < n; i++) {
    //    for (int j = 0; j < i; j++) {                        
